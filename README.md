...
`add-story.py` reads the linked page's title, description, publication, date, and social image, then adds an editable entry to `content/stories.json`.
...
The build creates 320px and 640px WebP thumbnails in `img/optimized`. Animated sources remain animated, and unchanged images are reused on later builds.
